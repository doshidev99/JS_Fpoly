...
class UserController {
    constructor(element) {
        ...
        btn === null || btn === void 0 ? void 0 : btn.addEventListener('click', this.processPlayBtnClick);
    }
    // log status class when process...
    processPlayBtnClick(e) {
        e.preventDefault();
        const that = this.element;
        const form = that.querySelector('form');
        const usernameElm = that.querySelector('#username');
        const helpId = that.querySelector('#help-username');
        if (usernameElm) {
            let user = new _models_User__WEBPACK_IMPORTED_MODULE_2__[/* User */ "a"](usernameElm.value);
            Object(class_validator__WEBPACK_IMPORTED_MODULE_1__[/* validate */ "c"])(user).then(errs => {
                if (errs.length > 0) {
                    if (helpId) {
                    ...
                    window.location.href = "http://localhost:3001/board";
                }
            });
        }
    }
...
], UserController.prototype, "processPlayBtnClick", null);
...
if (appEl) {
    let usercontroller = new _controllers_UserController__WEBPACK_IMPORTED_MODULE_0__[/* UserController */ "a"](appEl);
}
